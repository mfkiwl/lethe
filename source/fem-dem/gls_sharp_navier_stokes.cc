          ...
          if (iterator == principal_fluid_map.end())
            {
              ...
              cells_inside_map[cell] = {true,
                                        0}; // TODO CHOOSE A BETTER INTEGER
                                            // Loop over that group of cells
              // We also loop around this cell to convert the neighboring cut
              ...
              std::vector<typename DoFHandler<dim>::active_cell_iterator>
                active_neighbors_set =
                  LetheGridTools::find_cells_around_cell<dim>(vertices_to_cell,
                                                              cell);
              std::vector<typename DoFHandler<dim>::active_cell_iterator>
                     cut_neighbors_vector;
              size_t number_of_neighbors_checked = 0;
              bool   still_finding_neighbors     = true;
              while (still_finding_neighbors)
                {
                  still_finding_neighbors = false;
                  for (unsigned int i = 0; i < active_neighbors_set.size(); ++i)
                    {
                      auto neighbor_cell = active_neighbors_set[i];
                      bool neighbor_cell_is_cut;
                      int  particle_id;
                      int  count_particles;
                      std::tie(neighbor_cell_is_cut,
                               particle_id,
                               count_particles) = cut_cells_map[neighbor_cell];
                      if (neighbor_cell_is_cut)
                        {
                          still_finding_neighbors = true;
                          cut_neighbors_vector.push_back(neighbor_cell);
                          cells_inside_map[neighbor_cell] = {true, 0};
                          cut_cells_map[neighbor_cell]    = {false,
                                                          particle_id,
                                                          count_particles};
                        }
                    }
                  if (number_of_neighbors_checked < cut_neighbors_vector.size())
                    still_finding_neighbors = true;
                  if (still_finding_neighbors)
                    {
                      active_neighbors_set =
                        LetheGridTools::find_cells_around_cell<dim>(
                          vertices_to_cell,
                          cut_neighbors_vector[number_of_neighbors_checked]);
                      number_of_neighbors_checked += 1;
                    }
                }
            }
        }
